/**
 * lang-switcher.js — Language switcher for the Intro to AI Harness Handbook
 *
 * Provides a dropdown that lets users switch between Korean (default)
 * and English translations of each page.
 *
 * Behaviour:
 * - On load: reads localStorage('lang') and sets the matching option.
 * - Detects the current page language from the URL suffix (_en).
 * - Uses fetch(url, { method: 'HEAD' }) to check whether _en.html
 *   variants exist; caches results and disables options for unavailable
 *   languages.
 * - On change: strips any existing _en suffix from the current URL,
